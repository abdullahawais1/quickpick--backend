...
const SCHOOL_LOCATION = { latitude: 33.6844, longitude: 73.0479 };
...
export const autoJoinQueue = asyncHandler(async (req: Request, res: Response) => {
...
export const pickupComplete = asyncHandler(async (req: Request, res: Response) => {
  const userEmail = req.user?.email;
...
export const getQueueWithStudents = asyncHandler(async (req: Request, res: Response) => {
  const queue = await QueueEntry.find().sort({ queueNumber: 1 });
